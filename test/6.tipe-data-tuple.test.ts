...
it('test readonly', () => {
    ...
    let d:readonly [number, string] = [1,'rafa']
    ...
    let array2:Readonly<[number, string, number[], boolean, ...number[]]> = [1,'rafa',[10,10,10],true, 20,20,20,20] 
    console.log(array2)
...
let identitas: [string, number] = ["Rafa", 25]
...
let data: [string, boolean] = ["Login berhasil", true]
...
let dataPengguna: [string, number?] = ["Andi"]
...
let logData: [string, ...number[]] = ["Total", 100, 200, 300]
...
let user: [name: string, age: number] = ["Citra", 30]

// Tuple dan Type Alias
type Hasil = [string, boolean]
let hasilProses: Hasil = ["Berhasil", true]
...
let tupleCampuran: [string | number, boolean] = [123, true]
tupleCampuran = ["abc", false] // juga valid

// Tuple dengan array di dalamnya
let tabel: [string, number[]] = ["Statistik", [10, 20, 30]]
