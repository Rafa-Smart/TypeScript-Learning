...
const obj: any = JSON.parse(raw); // bisa, tapi lebih baik didefinisikan

// ===================================================================
// * CARA MENGHINDARI "any"
// ===================================================================
// ✅ Gunakan tipe eksplisit saat memungkinkan:
type User = {
  id: number;
  nama: string;
};

const dataUser: User = JSON.parse(raw); // ✅ lebih aman

// Atau gunakan `unknown`:
// const hasil: unknown = JSON.parse(raw);
if (typeof hasil === "object" && hasil !== null && "nama" in hasil) {
  console.log((hasil as any).nama);
}
