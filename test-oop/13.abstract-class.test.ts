...
describe("testing", () => {
  ...
  abstract class Customer {
    ...
    // nah berati nanti kalo ada yg extends Customer
    // maka wajib dia mempunyai property nama dan method sayHello ini
    // dan juga dia akan bisa mengkases property id dan method hello

    // abstract sayHello:(nama:string) => string;
    // ini salah karena ini tuh, sama aja kayak kita ingin mendeklarasikan property yg bertipe method
    // bukan method
    // jadi property sayHello bertipe method (nama:string) => string

    // nah yang benar adlah
    abstract sayHello(nama: string): string;
  }

  class RegulerCustomer extends Customer {

    // disini ktia wajib mengimplementasi
    // seluruh hal hal yang di abstraksi di Customer
  ...
  const jamal: Customer = new RegulerCustomer("jamal", 1);
  ...
  abstract class Kendaraan {
    ...
    // Method abstract: hanya deklarasi, tidak punya isi
    abstract nyalakan(): void;
  ...
  const kendaraan1: Kendaraan = new Mobil("Toyota");
  const kendaraan2: Kendaraan = new Motor("Honda");
  ...
  abstract class Pembayaran {
    protected nominal: number;

    constructor(nominal: number) {
      this.nominal = nominal;
    }

    abstract proses(): void;
  ...
  class PembayaranQRIS extends Pembayaran {
    proses(): void {
      console.log(
        `Pembayaran QRIS sejumlah Rp${this.nominal} sedang diproses...`
      );
    }
  }
  ...
  function lakukanPembayaran(p: Pembayaran) {
    p.info();
    p.proses();
  }
