it("testing", () => {
  ...
  function combine<T, U>(a: T, b: U): { first: T; second: U } {
    return { first: a, second: b };
  }

  // Pemakaian:
  const hasil1 = combine<number, string>(10, "sepuluh"); // T = number, U = string
  const hasil2 = combine<boolean, number>(true, 99); // T = boolean, U = number
  ...
  // ---------------------------------------------
  function mapValue<K, V>(key: K, value: V): Map<K, V> {
    const map = new Map<K, V>();
    map.set(key, value);
    return map;
  }

  const myMap = mapValue<number, string>(1, "satu");
  ...
  // Bisa juga digunakan untuk menyimpan pasangan relasi:
  type Pair<K, V> = {
    key: K;
    value: V;
  };
  ...
  const pasangan = makePair<string, number>("usia", 25);

  // Penambahan default generic type:
  // -------------------------------
  // Kita bisa memberikan default untuk type parameter:
  function withDefault<T = string, U = number>(a: T, b: U): [T, U] {
    return [a, b];
  }
